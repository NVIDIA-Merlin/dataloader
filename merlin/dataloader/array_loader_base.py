...
class ArrayLoaderBase:
    ...
    def make_tensors(self, gdf, use_row_lengths=False):
        ...
        """
        tensors_by_name = self.convert_df_to_tensors(gdf)
        rows_per_batch = self._get_rows_per_batch(len(gdf))

        # {"col_name": [list of batches]}
        tensor_batches = {}
        ...
        for tensor_key, tensor_value in tensors_by_name.items():
            if isinstance(tensor_value, tuple):
                # List feature
                # Could be ragged, so we need to process these in a way that works for ragged
                # But we don't really care if it is ragged, just that it could be
                ...
                tensor_batches[tensor_key] = _split_values(tensor_value, num_values_per_batch)
    ...
    def convert_df_to_tensors(self, gdf):
